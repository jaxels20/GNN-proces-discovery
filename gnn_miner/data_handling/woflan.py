...
from pm4py.algo.analysis.woflan.algorithm import compute_place_invariants
from pm4py.algo.analysis.woflan.algorithm import transform_basis
from pm4py.algo.analysis.woflan.algorithm import compute_s_components
from pm4py.algo.analysis.woflan.algorithm import compute_uncovered_places_in_component
from pm4py.algo.analysis.woflan.algorithm import compute_uncovered_place_in_invariants
...
def step_2_(woflan_object, return_asap_when_unsound=False, already_short_circuited=True):
  ...
  wof_places = woflan_object.net.places
  wof_transitions = woflan_object.net.transitions
  to_discover = wof_places.union(wof_transitions)
...
def step_3_(woflan_object, return_asap_when_unsound=False):
  woflan_object.set_place_invariants(compute_place_invariants(woflan_object.net))
  ...
  woflan_object.set_s_components(
    compute_s_components(woflan_object.net, woflan_object.get_uniform_place_invariants()))
  woflan_object.set_uncovered_places_s_component(
    compute_uncovered_places_in_component(woflan_object.get_s_components(), woflan_object.net))
